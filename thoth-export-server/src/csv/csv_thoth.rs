...
struct CsvThothRow {
    ...
    #[serde(
        rename = "publications [(type, isbn, width (mm), width (cm), width (in), height (mm), height (cm), height (in), weight (g), weight (oz), [(ISO_4217_currency, price)], [(landing_page, full_text, platform, is_canonical)])]"
    )]
    publications: String,
    #[serde(
    ...
    funding: String,
    #[serde(rename = "relations [(related_work, relation_type, ordinal)]")]
    relations: String,
}
...
impl CsvCell<CsvThoth> for WorkRelations {
    fn csv_cell(&self) -> String {
        format!(
            "(\"{}\", \"{:?}\", \"{}\")",
            self.related_work.full_title, self.relation_type, self.relation_ordinal,
        )
    }
}
...
"#;
    ...
    fn test_csv_thoth_relations() {
        ...
        assert_eq!(
            ...
            r#"("Related work title", "HAS_CHILD", "1")"#.to_string()
        );
