...
impl TryFrom<Work> for KbartOclcRow {
    ...
    fn try_from(work: Work) -> ThothResult<Self> {
        ...
        let landing_page = work.landing_page.as_ref().ok_or_else(|| {
        ...
        let publication_date = work.publication_date.as_ref().ok_or_else(|| {
            ThothError::IncompleteMetadataRecord(
                KBART_ERROR.to_string(),
        ...
        work.license.as_ref().ok_or_else(|| {
            ThothError::IncompleteMetadataRecord(
                KBART_ERROR.to_string(),
