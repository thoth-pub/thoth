...
use self::handler::{by_publisher, by_work, by_work_prepared, get_all, get_one};
...
pub(crate) fn route(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/specifications")
            ...
            .service(
                web::resource("/{specification_id}/work/{work_id}/prepared/{crossref_timestamp}")
                    .route(web::get().to(by_work_prepared)),
            )
            .service(
