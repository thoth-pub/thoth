...
Last updated: 2026-09-02 (`MET-WP1-08`, issue
[#882](https://github.com/thoth-pub/thoth/issues/882): the OPERAS mapping
persistence foundation slice — the `metric_operas_mapping` table — is
implemented on its slice branch `feature/metrics--wp1-operas-mapping`; the
mapping foundation remains additive and inactive: no mapping row is seeded and
no real OPERAS event, measure, uploader, platform or measure mapping value is
approved or guessed, because `docs/metrics/source-inventory.md` still records
them as unresolved external inputs; the `metric_operas_export`,
...
are deliberately not created by this slice, and no OPERAS payload
construction, delivery, claiming, lease, attempt, retry, backoff or status
state machine, remote event ID, request hash, delivery error, inbound
synchronization, loop prevention, reconciliation, cursor/snapshot discovery,
outbound eligibility or `METRICS_OPERAS_EXPORT` capability enforcement,
GraphQL/admin surface, Sphynx change or production migration was implemented;
`mapping_id` is a surrogate `UUID` primary key supplying the referential
target the design's later `metric_operas_export.mapping_id` implies,
`UNIQUE(platform_id, measure_id)` keeps one canonical mapping per registered
pair, a single non-cascading composite foreign key against
`metric_platform_measure (platform_id, measure_id)` prevents a mapping naming
a real platform and a real measure that are not registered together, the three
URI columns carry only the existing nonblank required-text CHECK with no
invented URI semantics, `enabled` has no database default and no timestamp or
duplicated `direct_collection` column is added; ADR-0001 remains the
entitlement authority and no Metrics-specific entitlement table is created;
the `MET-WP1-07` rollup-delta persistence foundation (issue
...
`6093f0ca7f3b7221c656bf514d71b5812e39ac45`, which is the exact authorized base
of this slice; the `MET-WP1-06` publisher-platform approval foundation (issue
...
unapproved; later Sphynx/client/source/WP5 gates unchanged; no
