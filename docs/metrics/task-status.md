...
Last updated: 2026-09-03 (`MET-WP1-09`, issue
[#884](https://github.com/thoth-pub/thoth/issues/884): the OPERAS export
ledger persistence foundation slice — the `metric_operas_export` table — is
implemented on its slice branch `feature/metrics--wp1-operas-export`; the
export ledger is the durable idempotency and audit boundary a later external
OPERAS write must be reconciled against, and it remains additive and inactive:
no export row is seeded or created at runtime, and no outbound eligibility,
source-period finalization, `METRICS_OPERAS_EXPORT` capability enforcement,
export status vocabulary or transition graph, claim owner, lease, claim token,
retry schedule, backoff, stale-claim recovery or attempt-increment behaviour,
payload construction, normalization or hashing, OPERAS network call, remote
event recording, correction/divergence handling, inbound synchronization,
reconciliation, GraphQL/admin surface, Sphinx change or production migration
was implemented; the table carries exactly the ten design-named fields, with
`export_id` a surrogate `UUID` primary key on the repository-standard Metrics
UUID default, `UNIQUE(record_revision_id)` permitting at most one durable
export row per canonical revision, two single-column non-cascading foreign
keys to `metric_record_revision (record_revision_id)` and
`metric_operas_mapping (mapping_id)`, `status` required `TEXT` with only the
nonblank required-text CHECK and no enum, vocabulary or default,
`attempt_count` required `INTEGER` with a non-negative CHECK and no default,
nullable opaque `remote_event_id` and `request_hash` that are nonblank when
supplied but carry no invented syntax, algorithm or uniqueness rule, nullable
`last_error`, the standard current-time `created_at` default and a nullable
`completed_at` with no status tie, and an index set of exactly the primary key
and the revision-uniqueness index; the approved design's section 6.14 names no
retry-time field while its section 14.4 refers to export indexes on status and
retry time, and that reviewed inconsistency is a deliberate deferral to WP9,
which owns the eventual claim/retry representation and its query/index
contract, so no `retry_at`, `next_attempt_at`, lease or claim column and no
speculative status/retry index is introduced; mapping-to-revision
platform/measure correspondence likewise remains a later fail-closed
enqueue/eligibility validation rather than redundant columns or a trigger; the
...
no Metrics-specific entitlement table is created; the `MET-WP1-08` OPERAS
mapping persistence foundation (issue
...
`d4980e6fb3ff6a08acebb95c7cb87306750469f2`, which is the exact authorized base
of this slice; the `MET-WP1-07` rollup-delta persistence foundation (issue
...
|---|---|---|---:|---|---|---|
| WP1 | Domain and database foundation | `thoth` | HIGH | IN PROGRESS | entry gates satisfied; registry foundation (`MET-WP1-01`), source-state foundation (`MET-WP1-02`), import-state foundation (`MET-WP1-03`), record-history foundation (`MET-WP1-04`) and coverage foundation (`MET-WP1-05`) merged/delivered to `feature/metrics`; publisher-platform approval foundation (`MET-WP1-06`) and rollup-delta persistence foundation (`MET-WP1-07`) merged to `feature/metrics`; OPERAS mapping persistence foundation (`MET-WP1-08`) implemented on its slice branch `feature/metrics--wp1-operas-mapping`; each remaining slice requires its own approved bounded specification and separate authorization | #766 |
