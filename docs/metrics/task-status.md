...
Last updated: 2026-09-02 (`MET-WP1-06`, issue
[#878](https://github.com/thoth-pub/thoth/issues/878): the publisher-platform
approval foundation slice — `metric_publisher_platform_approval` and the
closed `metric_publisher_platform_approval_status` enum — is implemented on
its slice branch `feature/metrics--wp1-publisher-approval`; the approval
foundation remains additive and inactive: no approval row is seeded, and
no approval creation/transition/revocation service, `PUBLISHER_CONTROLLED`
platform-ownership enforcement, package/capability entitlement check,
publisher-import authorization, GraphQL/admin surface or production
migration was implemented; `approved_by` remains nullable `UUID` with no
foreign key and no actor-identity semantics, deferred to a later separately
reviewed administrative write-path specification; the `MET-WP1-05` coverage
foundation (issue [#875](https://github.com/thoth-pub/thoth/issues/875)) is
...
|---|---|---|---:|---|---|---|
| WP1 | Domain and database foundation | `thoth` | HIGH | IN PROGRESS | entry gates satisfied; registry foundation (`MET-WP1-01`), source-state foundation (`MET-WP1-02`), import-state foundation (`MET-WP1-03`), record-history foundation (`MET-WP1-04`) and coverage foundation (`MET-WP1-05`) merged/delivered to `feature/metrics`; publisher-platform approval foundation (`MET-WP1-06`) implemented on its slice branch `feature/metrics--wp1-publisher-approval`; each remaining slice requires its own approved bounded specification and separate authorization | #766 |
