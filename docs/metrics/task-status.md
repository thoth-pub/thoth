...
`metric_coverage_status` enum — is implemented on
`feature/metrics--wp1-coverage` and **not merged**; draft PR
[#876](https://github.com/thoth-pub/thoth/pull/876) targets
`feature/metrics`; the `MET-WP1-04` record-history foundation (issue
...
|---|---|---|---:|---|---|---|
| WP1 | Domain and database foundation | `thoth` | HIGH | IN PROGRESS | entry gates satisfied; registry foundation (`MET-WP1-01`), source-state foundation (`MET-WP1-02`), import-state foundation (`MET-WP1-03`) and record-history foundation (`MET-WP1-04`) merged/delivered to `feature/metrics`; coverage foundation (`MET-WP1-05`) implemented on its slice branch with draft PR #876 open, not yet merged; each remaining slice requires its own approved bounded specification and separate authorization | #766 |
