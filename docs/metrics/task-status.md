...
Last updated: 2026-09-06 (`MET-WP1-11`, issue
[#890](https://github.com/thoth-pub/thoth/issues/890): the reconciliation
ledger persistence foundation slice — the `metric_reconciliation_run` and
`metric_reconciliation_issue` tables — is implemented on its slice branch
`feature/metrics--wp1-reconciliation-ledger`; the two tables land together
because an issue belongs to exactly one run and the two form one durable audit
unit, and they remain additive and inactive: no reconciliation row is seeded or
created at runtime, and no reconciliation execution, comparison of source
manifests, canonical records, rollups or OPERAS ledgers, issue classification,
status/type/severity vocabulary, resolution or reopening workflow, OPERAS loop
prevention, divergence handling, snapshot or rolling scan, completeness
determination, claim, lease, retry or backoff behaviour,
`recordMetricReconciliation` or other GraphQL/admin surface, Sphinx change or
production migration was implemented; `metric_reconciliation_run` carries
exactly the six design-named fields, with a surrogate `UUID` `run_id`, required
schemaless `JSONB` `scope` with no default because a run must state what it
covered, required opaque `TEXT` `status` carrying only the nonblank
required-text CHECK with no enum, vocabulary, default or transition graph, a
required `started_at` that deliberately has **no** database default because it
is the actual reconciliation-execution start supplied by the writer rather than
Thoth's insertion time — so an omitted value fails and a supplied value
round-trips exactly — a nullable `completed_at` with no default and no
status-dependent CHECK, and a required `JSONB` `summary` defaulting to `'{}'`
with no required keys; `metric_reconciliation_issue` carries exactly the eight
design-named fields, with a surrogate `UUID` `issue_id` and no inferred
uniqueness over `(run_id, issue_type, record_id, remote_event_id)`, a required
non-cascading `run_id` foreign key so deleting a run while its evidence exists
fails rather than cascading, required opaque `TEXT` `issue_type` and `severity`
because the design's prose examples are illustrative rather than an exhaustive
enum, a nullable non-cascading `record_id` foreign key to
`metric_record (record_id)` because unexpected remote records, unmapped
measures and unresolved works can exist before or without a canonical record, a
nullable opaque `remote_event_id` that deliberately has **no** foreign key and
no global uniqueness because `MET-WP1-10` established canonical remote identity
as the composite `(remote_instance, remote_event_id)` and reconciliation may
also refer to outbound or legacy remote evidence, a required `JSONB` `details`
defaulting to `'{}'`, and a nullable `resolved_at` with no resolution-workflow
column or state machine; the index inventory is exactly the two primary keys,
with no secondary index on `run_id`, `record_id`, `remote_event_id`,
`issue_type`, `severity` or `resolved_at`, because PostgreSQL needs no
child-side referencing index to enforce these foreign keys, the merged
`metric_import_error (import_id)` precedent likewise carries none and no
reconciliation query plan exists yet, so WP9 may add operational indexes only
from actual access patterns with query-plan evidence; persisting reconciliation
state does not solve the design's section 15.5 OPERAS inbound-completeness
blocker, which remains externally unresolved without an adequate
cursor/created-at stream, replication, a complete snapshot or export, or an
equivalent reliable incremental mechanism, so no completeness, coverage,
cursor, scan or snapshot field is added and WP9 retains ownership of
completeness reporting; ADR-0001 remains the entitlement authority and no
Metrics-specific entitlement table is created; the `MET-WP1-10` OPERAS import
ledger persistence foundation (issue
[#888](https://github.com/thoth-pub/thoth/issues/888)) is merged into
`feature/metrics` through PR
[#889](https://github.com/thoth-pub/thoth/pull/889), merge commit
`27e0811ada32b38ef43458c47db5c3c94e4b4680`, which is the exact authorized base
of this slice; the `MET-WP1-09` OPERAS export ledger persistence foundation
(issue [#884](https://github.com/thoth-pub/thoth/issues/884)) is merged into
`feature/metrics` through PR
[#886](https://github.com/thoth-pub/thoth/pull/886), merge commit
`f66048fcc3f4eacf8c5ce1ac9c07e9fa3179eb0f`; the `MET-WP1-08` OPERAS mapping
...
|---|---|---|---:|---|---|---|
| WP1 | Domain and database foundation | `thoth` | HIGH | IN PROGRESS | entry gates satisfied; registry foundation (`MET-WP1-01`), source-state foundation (`MET-WP1-02`), import-state foundation (`MET-WP1-03`), record-history foundation (`MET-WP1-04`) and coverage foundation (`MET-WP1-05`) merged/delivered to `feature/metrics`; publisher-platform approval foundation (`MET-WP1-06`), rollup-delta persistence foundation (`MET-WP1-07`), OPERAS mapping persistence foundation (`MET-WP1-08`) and OPERAS export ledger persistence foundation (`MET-WP1-09`) and OPERAS import ledger persistence foundation (`MET-WP1-10`) merged to `feature/metrics`; reconciliation ledger persistence foundation (`MET-WP1-11`) implemented on its slice branch `feature/metrics--wp1-reconciliation-ledger`; each remaining slice requires its own approved bounded specification and separate authorization | #766 |
