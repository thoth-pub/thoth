...
Last updated: 2026-09-02 (`MET-WP1-07`, issue
[#880](https://github.com/thoth-pub/thoth/issues/880): the rollup-delta
persistence foundation slice — the `metric_rollup_delta` table — is
implemented on its slice branch `feature/metrics--wp1-rollup-delta`; the
rollup-delta foundation remains additive and inactive: no delta row is
seeded, and no delta generation from ingestion, first-arrival/revision/
retraction transaction, delta claiming, lease, retry, backoff or stale-claim
recovery, `FOR UPDATE SKIP LOCKED` application loop, delta application,
rollup projection table, rebuild generation, active-watermark behaviour,
`claimMetricRollupDeltas`/`completeMetricRollupDeltas` mutation, dashboard
query, GraphQL/admin surface or production migration was implemented;
`delta_value` is a signed `BIGINT` with no non-negative CHECK, `status` is
required `TEXT` with no closed vocabulary, enum, CHECK or trigger, and
`applied_at` is nullable with no cross-column invariant, all deferred to the
later bounded WP4 claim/application specification; the four rebuildable
work-level rollup projection tables remain approved future architecture and
are deliberately not created by this slice; the `MET-WP1-06`
...
`6a768a5bf60ad9fa00757b19faea633e1ca21c08`, which is the exact authorized
base of this slice; the `MET-WP1-05` coverage foundation (issue
...
|---|---|---|---:|---|---|---|
| WP1 | Domain and database foundation | `thoth` | HIGH | IN PROGRESS | entry gates satisfied; registry foundation (`MET-WP1-01`), source-state foundation (`MET-WP1-02`), import-state foundation (`MET-WP1-03`), record-history foundation (`MET-WP1-04`) and coverage foundation (`MET-WP1-05`) merged/delivered to `feature/metrics`; publisher-platform approval foundation (`MET-WP1-06`) merged to `feature/metrics`; rollup-delta persistence foundation (`MET-WP1-07`) implemented on its slice branch `feature/metrics--wp1-rollup-delta`; each remaining slice requires its own approved bounded specification and separate authorization | #766 |
