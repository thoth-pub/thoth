...
| `metrics-dashboard` | repository-local root `AGENTS.md` merged onto `dev` through PR [#10](https://github.com/thoth-pub/metrics-dashboard/pull/10) and verified live 2026-08-16 at `963b0ea78a9a65153ab7d78b7c26e3cb35d763f4` | none for instruction rollout; the merged file is authoritative on `dev`, which is the repository's approved permanent development branch (`feature/* -> dev -> main`; no `dev -> develop` normalization). The remaining BR-DASH-01 gates — cleanup of the stale legacy `develop` ref (not deleted; deletion separately authorized), branch protection, and its absent CI, tests and lint gate ([CG-11](../repository-map/control-gaps.md#cg-11---ci-gaps)) — remain separate |
