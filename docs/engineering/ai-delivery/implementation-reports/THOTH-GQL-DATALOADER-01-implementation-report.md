...
- `thoth-api/Cargo.toml`
  - reason: add `dataloader = { version = "0.18", default-features = false, features = ["runtime-tokio"] }` and direct normal `tokio` (`rt`) for the production `spawn_blocking` boundary; dev-only Tokio gains `time` for the delayed-cohort scheduling fixture.
