...
| `ADR-0010` | [Staff Operations Console and canonical publisher-service operations](ADR-0010-staff-operations-console.md) | PROPOSED | Staff Operations Console; Publisher Services and Distribution Configuration; OAI-PMH; Thoth Metrics; shared backend/authorization architecture | Requires fresh independent exact-head review, explicit CTO approval of the exact written decision, and repository-authoritative merge before implementation may rely on it. The proposal authorizes no implementation, migration, provider/runtime action, external write, replay, deployment, release or production activation. |
