...
the same ZITADEL project. At the ADR's verification base the repository has no
machine role at all, so whichever programme landed first would have set the
precedent for the other by delivery order rather than by decision.
