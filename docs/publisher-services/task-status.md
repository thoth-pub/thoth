...
10. BE-03's BE-01 and BE-02 dependencies are satisfied. The written
   ...
   [BE-03/BE-04 phase boundary](decisions.md), under which BE-03 owns desired
   configuration only and creates no durable job, no placeholder job and no
   fabricated job status — which requires a CTO decision. BE-03 implementation
   is `NOT AUTHORIZED` and additionally requires separate explicit
