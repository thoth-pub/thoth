...
8. BE-02's ADR-01 dependency is satisfied: the ADR-01 implementation was
   ...
   reachable from `develop`. The specification escalates one unresolved
   architecture decision to the CTO: how `Publisher.distributionPlatforms`
   satisfies the `thoth-api/AGENTS.md` section 6 N+1 rule, given that the
   repository provides no existing batching or set-based child-field
   mechanism. BE-02 is HIGH risk and remains `BLOCKED` and unauthorized:
   implementation still cannot start, and it requires that approved bounded
   specification, the recorded CTO decision on that gate, and fresh explicit
   implementation authorization from the then-current exact `develop` head. No BE-02 runtime
...
11. Beyond the delivered and merged documentation-only ADR-01 implementation
    and its ADR-01-CLOSEOUT-01 control reconciliation, no BE-02, BE-03,
    BE-04, APP-01, OAI-PMH, deployment, release or production work is
    authorized; all licence, migration, app, dissemination and operational
    tasks remain blocked under their recorded dependencies. Neither the
    merged ADR-01 implementation nor the repository-authoritative ADR-0004
    and final inventory authorizes runtime change, credential use, workflow
    dispatch or production access.
