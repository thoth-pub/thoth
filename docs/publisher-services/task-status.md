...
Last updated: 2026-08-15 (BE-04 reaches a two-state specification position, and the two must not be conflated. The **approved baseline** — the BE-04 specification as merged into `develop` through PR [#814](https://github.com/thoth-pub/thoth/pull/814) — is CTO-approved and repository-authoritative, and the CTO separately and explicitly authorized BE-04 implementation on that pull request against the freshly verified base `develop` at `ed32712766c8f5a1951bb53ec3192e18f067c7d2`; the implementation candidate on `feature/publisher-services/be-04` was created under that authorization and is properly authorized work. **`BE-04-SPEC-ADDENDUM-01`**, authored against the same base, carries corrected specification content whose approval authority is the CTO and whose authority condition is that the exact CTO-approved content be reachable from `develop`. It reconciles three defects a fresh independent review found by measuring the candidate against the baseline: a non-NULL-safe attempt-error `CHECK`, an unsatisfiable exact report statement-count contract, and a mandatory `cargo test -p thoth-client` gate that does not build at the authorized base. The implementation candidate is therefore **BLOCKED**: it was built against the baseline contract and does not satisfy the corrected one — not because it was unauthorized. The baseline authorization remains valid history and is insufficient for implementation against the corrected contract, which requires the corrected content to be repository-authoritative on `develop`, a freshly verified base and a **new** explicit CTO implementation authorization. Live review, approval and merge state for the pull requests carrying any of this is the GitHub record (`ADR-0005`). The addendum corrects requirements only: it approves nothing, authorizes no implementation, and neither extends nor withdraws the baseline authorization. Earlier state unchanged: BE-03 closed as an inactive merged foundation, with its dependency satisfied for BE-04, MIG-01, APP-01 and APP-02 without any of them becoming ready, and `ADR-0008` repository-authoritative on `develop`, satisfying that BE-04 control prerequisite without approving the BE-04 specification or authorizing implementation)
...
| [BE-04 Durable distribution jobs](../engineering/ai-delivery/tasks/BE-04.md) | `thoth` | HIGH | BLOCKED - IMPLEMENTATION CANDIDATE REQUIRES CORRECTED-CONTRACT RECONCILIATION | `develop` / `develop` | BE-02 (CLOSED, satisfied); BE-03 (CLOSED, satisfied); [`ADR-0008`](../engineering/decisions/ADR-0008-machine-roles-and-durable-job-primitives.md) machine roles and durable job primitives (**satisfied**: its exact approved content was independently reviewed at its exact head and merged into `develop` through PR [#815](https://github.com/thoth-pub/thoth/pull/815), so it is now repository-authoritative). Satisfying `ADR-0008` is a necessary and not a sufficient condition for BE-04 implementation authorization: `ADR-0008` approves `DISSEMINATION_WORKER` as a Publisher-Services-specific machine role under the shared least-privilege, domain-specific convention, and keeps BE-04's `distribution_job*` tables, Rust domain types and lifecycle APIs programme-local; it does **not** fix BE-04's operation-level authorization matrix, which the BE-04 specification still owns, it approves no BE-04 specification candidate, and it authorizes no BE-04 implementation. [`tasks/BE-04.md`](../engineering/ai-delivery/tasks/BE-04.md) now carries **two specification states, and neither erases the other**. The **approved baseline** is that document's pre-addendum content, which was CTO-approved and merged into `develop` through PR [#814](https://github.com/thoth-pub/thoth/pull/814), making it repository-authoritative; the CTO then separately and explicitly authorized BE-04 implementation on that pull request against the freshly verified base `develop` at `ed32712766c8f5a1951bb53ec3192e18f067c7d2`. **`BE-04-SPEC-ADDENDUM-01` carries corrected specification content; its approval authority is the CTO and its authority condition is that the exact CTO-approved content be reachable from `develop`.** Historical specification-authoring record, retained as history: three independent review rounds of the pre-merge specification candidate identified remediation requirements, and all three sets of corrections were made on the specification branch before it merged, together with the substantive `ADR-0008` reconciliation: the first round covered `OFF`-mode onboarding loss, the five-attempt budget, a repair being treated as evidence of delivery, and a claim statement that returned no rows; the second covered a false migration claim that no lock is taken on any existing table (BE-04's foreign keys do take `SHARE ROW EXCLUSIVE` locks on the existing `publisher` and `work` tables while they are established), contradictory `lastError` semantics, staff-report statement-count arithmetic that did not add up, role-composition wording that overreached `ADR-0008`, and an unspecified API error contract for an invalid worker `errorCode`; the third found no new architectural defect and two residual consistency defects created by the second round's own corrections — a stale "two to three" error-count sentence left in section 9.4.3 after the inventory became four variants, and a staff-report statement-count bound of six asserted against a selection that omits `enabledDistributionPlatforms` and therefore issues only five statements, now split into an explicitly measured five-statement job-only selection and six-statement full report selection. That third-round split is itself superseded by finding (b) below, which establishes that an exact statement-count contract was unsatisfiable in either form. That review-round history concerns the baseline, which has since been approved and merged. What now remains required applies to **addendum 01**: fresh exact-head independent review; explicit CTO approval of the corrected content; that approved content becoming reachable from `develop`; a freshly verified implementation base; and a **new** explicit CTO implementation authorization bound to that base and to the corrected contract. Authoring the specification changed no runtime code, created no migration and created no relation. On `develop` there is still no `distribution_job`, `distribution_job_target` or `distribution_job_attempt` relation, **no distribution job exists**, automatic job creation remains nonexistent and **not active** (the specified switch is OFF by default and nothing implementing it has merged), and no dissemination occurs. BE-03's merge created none of those relations, no automatic back-catalogue job creation and no dissemination either; the BE-03 write coordinator remains the seam BE-04 would later extend, not a partial BE-04 implementation. **A BE-04 implementation candidate exists** on `feature/publisher-services/be-04` as an unmerged pull request, created under the baseline implementation authorization above and therefore **properly authorized work**, not an approval of its content. It was measured against the baseline specification by a fresh independent review, which found three defects **in the specification** — (a) `distribution_job_attempt_error_result_check` was written as `(error_code IS NULL AND error_detail IS NULL) OR result = 'FAILED'`, which PostgreSQL admits when it evaluates to `UNKNOWN`, so an open attempt carrying error fields was accepted and the intended closure invariant was unenforced; (b) the exact five/six report statement-count contract is unsatisfiable, because two of the three specified DataLoaders were keyed by a value that exists only after an upstream loader resolves, making their dispatch count scheduler-dependent and provably bounded only by the page size; (c) the mandatory `cargo test -p thoth-client` gate does not build at the authorized base either, for a pre-existing repository packaging reason unrelated to BE-04. Those three are reconciled by `BE-04-SPEC-ADDENDUM-01`, which corrects requirements only, approves nothing, authorizes nothing, and neither extends nor withdraws the baseline authorization. The candidate is **BLOCKED** because it was built against the baseline contract and does not satisfy the corrected one, **not** because it was unauthorized. Implementation **against the corrected specification** requires the corrected content to be repository-authoritative on `develop`, a freshly verified `develop` head, and a **new** explicit CTO implementation authorization bound to that head; live satisfaction of those gates is the GitHub record | #765 | Specification [#814](https://github.com/thoth-pub/thoth/pull/814) (merged); implementation candidate [#816](https://github.com/thoth-pub/thoth/pull/816) on `feature/publisher-services/be-04`; specification addendum `BE-04-SPEC-ADDENDUM-01` [#817](https://github.com/thoth-pub/thoth/pull/817) on `feature/publisher-services/be-04-spec-addendum-01`. Live review, approval and merge state for both is the GitHub record | BLOCKED - IMPLEMENTATION CANDIDATE REQUIRES CORRECTED-CONTRACT RECONCILIATION - the baseline specification is CTO-approved and repository-authoritative through PR #814, and implementation candidate #816 was properly authorized against `develop` at `ed32712766c8f5a1951bb53ec3192e18f067c7d2`. That candidate was built against the baseline contract and does **not** satisfy the corrected `BE-04-SPEC-ADDENDUM-01` contract, so it is not delivered work for BE-04 as corrected. Continuation requires the corrected content to become repository-authoritative, a freshly verified `develop` base and a new explicit CTO implementation authorization bound to it. `ADR-0008` satisfied but not sufficient. Live review, approval, authorization and merge state is the GitHub record |
...
13. The cross-programme machine-role and durable-job questions raised by BE-04
    ...
    **satisfied**. Satisfying it is a necessary and not a sufficient condition:
    ...
    neither. BE-04 remains `HIGH` and `BLOCKED` under item 15, and the
    corrected specification content in `BE-04-SPEC-ADDENDUM-01` still requires
    its own independent review, any remediation that review requires, and
...
14. BE-04's BE-02, BE-03 and `ADR-0008` dependencies are **satisfied**, and the
    ...
    **approved baseline** and is repository-authoritative. The CTO then
    ...
    to that base. An implementation branch `feature/publisher-services/be-04` and a
    an unmerged implementation pull request exist; that is observed
    repository state and is **not** an authorization, an approval or a delivery,
    and the candidate is `BLOCKED` under item 15. The specification settles the
...
15. **`BE-04-SPEC-ADDENDUM-01` reconciles three defects that a fresh independent
    ...
    ordinary reason that both the requirements and the base have moved. Resuming
    BE-04 implementation against the corrected specification requires, in order:
    independent review of the addendum; explicit CTO approval of the corrected
    specification; merge into `develop`; fresh verification of the new `develop`
    head; and a **new** explicit CTO implementation authorization bound to that
    head.
