...
Last updated: 2026-08-14 (BE-03 closed as an inactive merged foundation; the BE-03 dependency recorded as satisfied for BE-04, MIG-01, APP-01 and APP-02 without any of them becoming ready; `ADR-0008` merged into `develop` and now repository-authoritative, satisfying that BE-04 control prerequisite without approving the BE-04 specification candidate or authorizing implementation; the BE-04 specification candidate authored, remediated against three independent review rounds and reconciled with `ADR-0008`, and still `DRAFT` and **not approved**, with BE-04 still `BLOCKED` and BE-04 implementation still NOT AUTHORIZED)
...
14. BE-04's BE-02, BE-03 and `ADR-0008` dependencies are **satisfied**, and the
    ...
    base. `feature/publisher-services/be-04` does not exist and must not exist
    before then. The specification settles the durable job/target/attempt model,
    ...
    Authoring the candidate created **no** relation, migration, runtime code,
    GraphQL surface, role, branch or job: no `distribution_job`,
    `distribution_job_target` or `distribution_job_attempt` relation exists as a
    result of this work,
    **no distribution job exists**, automatic job creation remains nonexistent
    and **not active**, and
    no dissemination, deployment, migration execution or production action has
