...
| [ADR-01 Platform inventory/final architecture](../engineering/ai-delivery/tasks/ADR-01.md) | `thoth` | MEDIUM | BLOCKED | exact base recorded when the implementation branch is created from then-current `develop`; then-current `develop` / `develop` | written ADR-01 specification drafted (PROPOSED); independent specification review PENDING; explicit CTO approval of the written specification PENDING; specification approval and merge required before ADR-01 moves to `READY`. Implementation is NOT AUTHORIZED: creating `feature/publisher-services/adr-01` and making any implementation edit require separate explicit authorization after specification approval; the branch remains absent until then. ADR-01 is not blocked by BE-01. | [#765](https://github.com/thoth-pub/thoth/issues/765) | Specification [#780](https://github.com/thoth-pub/thoth/pull/780); TBD for implementation | SPECIFICATION DRAFTED - INDEPENDENT REVIEW PENDING - CTO SPECIFICATION APPROVAL PENDING - IMPLEMENTATION NOT AUTHORIZED - BRANCH ABSENT - final distribution-platform inventory remains provisional |
