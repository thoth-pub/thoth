...
14. BE-04's BE-02, BE-03 and `ADR-0008` dependencies are **satisfied**, and the
    ...
    report. It records one surfaced cross-programme **adjacency finding** for the
    CTO — Thoth Metrics WP5 "Service auth and entitlements" is separately blocked
    on a role decision in this same repository — and itself settles no
    shared service-role convention, creates no ADR and adds no programme
    decision; that shared machine-role convention is decided by `ADR-0008`
    under item 13, which approves no part of this specification candidate.
