...
Last updated: 2026-08-14 (BE-03 closed as an inactive merged foundation; the BE-03 dependency recorded as satisfied for BE-04, MIG-01, APP-01 and APP-02 without any of them becoming ready; `ADR-0008` recorded as a BE-04 control boundary, with BE-04 implementation still NOT AUTHORIZED)
...
| BE-04 Durable distribution jobs | `thoth` | HIGH | BLOCKED | `develop` / `develop` | BE-02 (CLOSED, satisfied); BE-03 (CLOSED, satisfied); [`ADR-0008`](../engineering/decisions/ADR-0008-machine-roles-and-durable-job-primitives.md) machine roles and durable job primitives, which must be repository-authoritative — independently reviewed at its exact head and merged into `develop` — before BE-04 implementation may be authorized, and which is a necessary and not a sufficient condition for that authorization. `ADR-0008` approves `DISSEMINATION_WORKER` as a Publisher-Services-specific machine role under the shared least-privilege, domain-specific convention, and keeps BE-04's durable job storage, Rust types, GraphQL operations, state machine and lifecycle API programme-local; it does **not** fix BE-04's operation-level authorization matrix, which the BE-04 specification still owns, and it approves no BE-04 specification candidate. BE-04 itself remains unspecified and unauthorized: its specification requires its own independent review, any remediation that review requires, and explicit approval before it is repository-authoritative, and implementation additionally requires separate explicit authorization from a freshly verified `develop` head. BE-03's merge created no `distribution_job`, `distribution_job_target` or `distribution_job_attempt` relation, no automatic back-catalogue job creation and no dissemination; automatic job creation remains nonexistent and inactive, and the BE-03 write coordinator is the seam BE-04 will later extend, not a partial BE-04 implementation | #765 | TBD | NOT STARTED |
...
13. The cross-programme machine-role and durable-job questions raised by BE-04
    ...
    distribution workflow; the durable-job and concurrency primitives are
    shared conventions rather than a shared job framework, so `FOR UPDATE SKIP
    LOCKED` and each other primitive must still be justified by the adopting
    task; and BE-04's `distribution_job`, `distribution_job_target` and
    `distribution_job_attempt` tables, Rust types, GraphQL operations, state
    machine and lifecycle API remain programme-local and may not be reused by
    another programme by analogy. Under `ADR-0008`'s authority condition, the
    decision is repository-authoritative only when its exact approved content
    is independently reviewed at its exact head and reachable from `develop`;
    it is not effective from an unmerged branch. `ADR-0008` must be
    repository-authoritative before BE-04 implementation may be authorized,
    and that is a necessary and not a sufficient condition: `ADR-0008` fixes no
    ...
    access. BE-04 implementation remains **NOT AUTHORIZED**, and the BE-04
