...
Last updated: 2026-08-15 (BE-04 specification addendum `BE-04-SPEC-ADDENDUM-01` authored against `develop` at `ed32712766c8f5a1951bb53ec3192e18f067c7d2`, reconciling three defects a fresh independent review found by measuring an implementation candidate against the specification: a non-NULL-safe attempt-error `CHECK`, an unsatisfiable exact report statement-count contract, and a mandatory `cargo test -p thoth-client` gate that does not build at the authorized base. A BE-04 implementation candidate now exists as a draft, unmerged pull request and is **BLOCKED** pending that addendum. The addendum corrects requirements only: it approves nothing, authorizes no implementation, and does not revive any earlier authorization. BE-04 remains `BLOCKED` and BE-04 implementation remains NOT AUTHORIZED. Earlier state unchanged: BE-03 closed as an inactive merged foundation, with its dependency satisfied for BE-04, MIG-01, APP-01 and APP-02 without any of them becoming ready, and `ADR-0008` repository-authoritative on `develop`, satisfying that BE-04 control prerequisite without approving the BE-04 specification or authorizing implementation)
...
| [BE-04 Durable distribution jobs](../engineering/ai-delivery/tasks/BE-04.md) | `thoth` | HIGH | BLOCKED - IMPLEMENTATION CANDIDATE EXISTS - SPECIFICATION ADDENDUM REQUIRED | `develop` / `develop` | BE-02 (CLOSED, satisfied); BE-03 (CLOSED, satisfied); [`ADR-0008`](../engineering/decisions/ADR-0008-machine-roles-and-durable-job-primitives.md) machine roles and durable job primitives (**satisfied**: its exact approved content was independently reviewed at its exact head and merged into `develop` through PR [#815](https://github.com/thoth-pub/thoth/pull/815), so it is now repository-authoritative). Satisfying `ADR-0008` is a necessary and not a sufficient condition for BE-04 implementation authorization: `ADR-0008` approves `DISSEMINATION_WORKER` as a Publisher-Services-specific machine role under the shared least-privilege, domain-specific convention, and keeps BE-04's `distribution_job*` tables, Rust domain types and lifecycle APIs programme-local; it does **not** fix BE-04's operation-level authorization matrix, which the BE-04 specification still owns, it approves no BE-04 specification candidate, and it authorizes no BE-04 implementation. **BE-04 implementation is NOT AUTHORIZED.** [`tasks/BE-04.md`](../engineering/ai-delivery/tasks/BE-04.md) is the BE-04 **specification candidate** and is `DRAFT` and **not approved**: it states requirements only, and its implementation authorization is recorded as `SEPARATE AND ABSENT`. Three independent review rounds of the candidate identified remediation requirements, and all three sets of corrections have been made on the specification branch, together with the substantive `ADR-0008` reconciliation: the first round covered `OFF`-mode onboarding loss, the five-attempt budget, a repair being treated as evidence of delivery, and a claim statement that returned no rows; the second covered a false migration claim that no lock is taken on any existing table (BE-04's foreign keys do take `SHARE ROW EXCLUSIVE` locks on the existing `publisher` and `work` tables while they are established), contradictory `lastError` semantics, staff-report statement-count arithmetic that did not add up, role-composition wording that overreached `ADR-0008`, and an unspecified API error contract for an invalid worker `errorCode`; the third found no new architectural defect and two residual consistency defects created by the second round's own corrections — a stale "two to three" error-count sentence left in section 9.4.3 after the inventory became four variants, and a staff-report statement-count bound of six asserted against a selection that omits `enabledDistributionPlatforms` and therefore issues only five statements, now split into an explicitly measured five-statement job-only selection and six-statement full report selection. That third-round split is itself superseded by finding (b) below, which establishes that an exact statement-count contract was unsatisfiable in either form. Fresh exact-head independent review and explicit CTO specification approval both remain required. Implementation requires all of — successful fresh independent review of that specification and any further remediation that review requires; exact CTO specification approval of that content; that exact approved content being reachable from `develop`; a freshly verified implementation base; and separate explicit CTO implementation authorization bound to that base. Authoring the specification changed no runtime code, created no migration, created no relation and created no branch: there is still no `distribution_job`, `distribution_job_target` or `distribution_job_attempt` relation, **no distribution job exists**, automatic job creation remains nonexistent and **not active** (the specified switch is OFF by default and is not implemented), and no dissemination occurs. BE-03's merge created none of those relations, no automatic back-catalogue job creation and no dissemination either; the BE-03 write coordinator remains the seam BE-04 would later extend, not a partial BE-04 implementation. **A BE-04 implementation candidate now exists** on `feature/publisher-services/be-04` as a draft, unmerged pull request. Its existence is observed repository state, not an approval: it was measured against this specification by a fresh independent review, which found three defects **in the specification** — (a) `distribution_job_attempt_error_result_check` was written as `(error_code IS NULL AND error_detail IS NULL) OR result = 'FAILED'`, which PostgreSQL admits when it evaluates to `UNKNOWN`, so an open attempt carrying error fields was accepted and the intended closure invariant was unenforced; (b) the exact five/six report statement-count contract is unsatisfiable, because two of the three specified DataLoaders were keyed by a value that exists only after an upstream loader resolves, making their dispatch count scheduler-dependent and provably bounded only by N; (c) the mandatory `cargo test -p thoth-client` gate does not build at the authorized base either, for a pre-existing repository packaging reason unrelated to BE-04. Those three are reconciled by `BE-04-SPEC-ADDENDUM-01`, which corrects requirements only and authorizes nothing. Until that addendum is independently reviewed, CTO-approved and merged, a new `develop` head is freshly verified and a new explicit CTO implementation authorization is bound to it, the candidate stays **BLOCKED** and BE-04 implementation stays **NOT AUTHORIZED** | #765 | Specification [#814](https://github.com/thoth-pub/thoth/pull/814) (merged); implementation candidate [#816](https://github.com/thoth-pub/thoth/pull/816) on `feature/publisher-services/be-04` - **draft, unmerged and BLOCKED**; specification addendum `BE-04-SPEC-ADDENDUM-01` on `feature/publisher-services/be-04-spec-addendum-01`. Live pull-request state is the GitHub record | BLOCKED - IMPLEMENTATION CANDIDATE EXISTS - SPECIFICATION ADDENDUM REQUIRED - an implementation candidate exists as a draft, unmerged pull request and is **not** delivered, ready, approved or merge ready. Three specification defects are open against it and are being corrected by `BE-04-SPEC-ADDENDUM-01`. `ADR-0008` satisfied but not sufficient; implementation **not authorized** |
...
13. The cross-programme machine-role and durable-job questions raised by BE-04
    ...
    **satisfied**. Satisfying it is a necessary and not a sufficient condition:
    ...
    access. BE-04 implementation remains **NOT AUTHORIZED**, BE-04 remains
    `HIGH` and `BLOCKED` and does not become `READY` merely because `ADR-0008`
    is satisfied, and the BE-04
    specification still requires its own independent review, any remediation
    that review requires, and explicit approval.
14. BE-04's BE-02, BE-03 and `ADR-0008` dependencies are **satisfied**, and the
    bounded BE-04 specification candidate
    [`tasks/BE-04.md`](../engineering/ai-delivery/tasks/BE-04.md) has been
    authored on `feature/publisher-services/be-04-spec`, whose specification
    pull request is [#814](https://github.com/thoth-pub/thoth/pull/814). It is
    `DRAFT`, it is **not** CTO-approved, and its
    implementation authorization is `SEPARATE AND ABSENT`. **BE-04
    implementation is NOT AUTHORIZED**, and becomes authorizable only when all
    five of the following hold: the specification passes independent review and
    receives any remediation that review requires; the exact specification
    content receives
    explicit CTO specification approval; that exact approved content is
    reachable from `develop`; an implementation base is freshly verified; and
    the CTO grants separate explicit implementation authorization bound to that
    base. An implementation branch `feature/publisher-services/be-04` and a
    ...
    of it. Independent review of the candidate identified remediation
    requirements — `OFF`-mode onboarding loss, the five-attempt budget, a repair
    being treated as delivery evidence, and a claim statement that returned no
    rows — which have been corrected on the specification branch together with
    the `ADR-0008` reconciliation; the specification is still **not approved**,
    and fresh exact-head independent review and explicit CTO specification
    approval both remain required. Three further specification defects, found by
    measuring the implementation candidate against the specification, are
    reconciled by `BE-04-SPEC-ADDENDUM-01` under item 15.
...
15. **`BE-04-SPEC-ADDENDUM-01` reconciles three defects that a fresh independent
    ...
    BE-04 specification.** They are specification defects, not only
    ...
    - **Finding B — the exact report statement-count contract was
      ...
      chunk, with the arithmetic stated exactly and the shared `ADR-0007`
      chunking property named as `ADR-0007`'s rather than restated as BE-04's.
    ...
    specification; it authorizes no implementation; and it does not revive the
    earlier implementation authorization, which was bound to a `develop` head
    that a merged specification correction supersedes. Resuming BE-04
    implementation requires, in order: independent review of the addendum;
    explicit CTO approval of the corrected specification; merge into `develop`;
    fresh verification of the new `develop` head; and a **new** explicit CTO
    implementation authorization bound to that head.
