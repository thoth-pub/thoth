...
Last updated: 2026-08-14 (BE-03 closed as an inactive merged foundation; the BE-03 dependency recorded as satisfied for BE-04, MIG-01, APP-01 and APP-02 without any of them becoming ready; the BE-04 specification candidate authored, with BE-04 implementation still NOT AUTHORIZED)
...
| [BE-04 Durable distribution jobs](../engineering/ai-delivery/tasks/BE-04.md) | `thoth` | HIGH | BLOCKED | `develop` / `develop` | BE-02 (CLOSED, satisfied); BE-03 (CLOSED, satisfied). **BE-04 implementation is NOT AUTHORIZED.** [`tasks/BE-04.md`](../engineering/ai-delivery/tasks/BE-04.md) is the BE-04 **specification candidate** and is `DRAFT`: it states requirements only, and its implementation authorization is recorded as `SEPARATE AND ABSENT`. Implementation requires all four of — exact CTO specification approval of that content; that exact approved content being reachable from `develop`; a freshly verified implementation base; and separate explicit CTO implementation authorization bound to that base. Authoring the specification changed no runtime code, created no migration, created no relation and created no branch: there is still no `distribution_job`, `distribution_job_target` or `distribution_job_attempt` relation, **no distribution job exists**, automatic job creation is **not active** (the specified switch is OFF by default and is not implemented), and no dissemination occurs. The BE-03 write coordinator remains the seam BE-04 would later extend, not a partial BE-04 implementation | #765 | Specification candidate on `feature/publisher-services/be-04-spec`; live pull-request state is the GitHub record | NOT STARTED - specification candidate authored; implementation not authorized |
...
12. Beyond the delivered and merged documentation-only ADR-01 implementation,
    ...
    production access.
13. BE-04's BE-02 and BE-03 dependencies are **satisfied**, and the bounded
    BE-04 specification candidate
    [`tasks/BE-04.md`](../engineering/ai-delivery/tasks/BE-04.md) has been
    authored on `feature/publisher-services/be-04-spec`. It is `DRAFT` and its
    ...
    four of the following hold: the exact specification content receives
    ...
    on a role decision in this same repository — and deliberately settles no
    shared service-role convention, creates no ADR and adds no programme
    decision. Authoring it created **no** relation, migration, runtime code,
    ...
    **no distribution job exists**, automatic job creation is **not active**, and
