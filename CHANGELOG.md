...
### Added
  ...
  - `MIG-01-LIC-NORM-01`: bounded deterministic licence-normalization tooling for the 24 reviewed representation-only Creative Commons deed/legalcode mappings (issue [828](https://github.com/thoth-pub/thoth/issues/828), parent programme [765](https://github.com/thoth-pub/thoth/issues/765)). A new narrow `thoth-api` administrative facade (`work::licence_normalization`) raw-byte-`SHA-256`-verifies the three immutable reviewed inputs (deterministic normalization manifest, non-executable 28-value manual-resolution register, and the exact bound `MIG-01` production manifest), mechanically enforces every reviewed representation-only invariant — canonical-target shape, prefix and deed/legalcode-suffix constraints (jurisdiction stripping is rejected), `cc-license` parser validation, and exact-string `SUPPORTED` disposition of all 9 distinct targets in the bound `MIG-01` manifest — and emits a deterministic, canonical, raw-byte-hashed dry-run plan (typed full-precision `updated_at` tokens serialized directly via serde) plus a bounded reconciliation report; apply then classifies every entry fail-closed (`PENDING`/`ALREADY_APPLIED_BY_THIS_PLAN`/`DRIFT`, with deleted or drifted Works and unplanned matching Works stopping the run before any write) and writes pending entries one Work per transaction under a `SELECT ... FOR UPDATE` row lock: exactly one normal `work_history` row from the complete pre-update Work with a plan-derived `MIG-01-LIC-NORM:<plan-sha256>` actor, and a single-column `work.license` update (no `Crud::update`, no `PatchWork`, no GraphQL `updateWork`), leaving timestamp freshness to the existing database triggers. A thin `thoth publisher-services licence-normalization dry-run|apply` CLI with explicit `disposable`/`production` execution modes (production requires the exact independently reviewed dry-run report hash) passes only paths and expected hashes; `cc_license 0.1.0` is added to `thoth-api` as an optional backend dependency. The 28 manual-resolution values remain unresolved and are reported only — they are never written and continue to block the `MIG-01` package/platform production apply. The expected read-time export/specification-cache regeneration load of licence writes is recorded in the report; no cache regeneration is triggered. No automatic production execution, no publisher package or distribution-platform change, no distribution jobs, and no migration, `schema.rs`, GraphQL/API contract or authorization change; no production data is read or written.
