...
## [Unreleased]
### Added
  - [788](https://github.com/thoth-pub/thoth/pull/788) - Specify the bounded BE-02 distribution platform model (specification only): add the `BE-02` task record required by Gate 1, settling every low-level representation choice from live repository conventions rather than the provisional values superseded by [ADR-0004](docs/engineering/decisions/ADR-0004-distribution-platform-inventory.md) - the `public.distribution_platform` PostgreSQL enum with exactly the 17 approved values in binding declaration order with no `OTHER`, fallback or `Default`; the closed Rust and GraphQL `DistributionPlatform` enum following the merged BE-01 `ThothPackage` pattern; the `publisher_distribution_platform` schema with its composite primary key, `ON DELETE CASCADE` foreign key, single-row lifecycle check constraint enforcing `enabled = (disabled_at IS NULL)` over `NOT NULL` `activation_id`/`enabled_at` columns so no never-activated row is representable, partial enabled reverse-lookup index, `set_updated_at` trigger, application-generated `activation_id` and transaction-timestamp lifecycle timestamps; all six activation-lifecycle transitions with their write, activation and idempotency semantics, retaining disabled rows and generating a new activation only on a genuine re-enable; group-level linked OAPEN/DOAB normalization in one transaction with one shared activation identity, identical timestamps, atomic rollback and no supported one-sided enabled state, enforced transactionally rather than through a new database trigger subsystem; `OCLC_KB` and `EX_LIBRIS_KB` as independent assignments sharing a feed profile without duplicate feed state or shared activations; code-owned exhaustive descriptors proved by a wildcard-free match, the `BackCatalogueBehaviour` vocabulary and an internal adapter profile representing shared mechanism identity without collapsing destinations; `JISC_NBK` visible but non-assignable and enforced fail-closed in the domain layer; and the exact public GraphQL contract inventory of four surfaces (three root query fields plus `Publisher.distributionPlatforms`), two object types and three GraphQL enums, on the repository's existing offset/limit pagination with a mandatory `publisher_id` tie-breaker and count/lookup agreement by construction. Also fixes the ADR-0003 Architecture A atomicity requirement, the empty and representative populated disposable-database migration evidence, the DDL lock assessment recording that establishing the foreign key takes a `SHARE ROW EXCLUSIVE` lock on the populated `publisher` table, blocking publisher writes while leaving reads unaffected, with mandatory `pg_locks` verification and no unevidenced production-duration claim, the retained-foundation operational rollback, the concrete required tests, the acceptance criteria and the stop conditions, and reconciles the Publisher Services tracker whose BE-02 row pointed at a non-existent specification and carried `TBD`. Documentation only - no BE-02 runtime code, migration, schema, model, GraphQL surface, descriptor, assignment table or test, no implementation branch, no ADR, inventory, evidence-ledger or evidence-count change, no modification of [issue #765](https://github.com/thoth-pub/thoth/issues/765), and no deployment, release, migration execution or activation; BE-02 remains blocked and unauthorized, and implementation requires this approved specification, a CTO decision on the one escalated architecture gate the specification carries (how `Publisher.distributionPlatforms` satisfies the `thoth-api/AGENTS.md` N+1 rule, for which the repository offers no existing batching or set-based mechanism), and separate explicit CTO authorization bound to a freshly verified exact `develop` head
