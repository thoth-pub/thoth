...
## [Unreleased]
### Added
  - [799](https://github.com/thoth-pub/thoth/pull/799) - `THOTH-GQL-OPS-03`: implement the approved effective-mode **fleet-verification mechanism**, supplying the implementation candidate for capability gap 2. The specification deliberately did **not** assert that a compliant mechanism existed inside its section 3.2 boundary and required `BLOCKED` rather than architecture widening if none did; the answer established here from repository evidence is **FEASIBLE**. Each serving process now emits **exactly one** structured record on its **own log stream** at startup — `THOTH_MUTATION_GUARD_EFFECTIVE_MODE mode=<OFF|OBSERVE|ENFORCE> instance=<id>` — read out of the **same stored `Data<MutationGuardMode>`** that is installed as the request path's `app_data`, so the reported mode and the mode the request path uses are one reference-counted value **by construction** and cannot be set independently: nothing re-reads `THOTH_GRAPHQL_MUTATION_GUARD_MODE`, re-parses configuration, maintains a second signal, or infers the mode from deployment intent or request behaviour. A companion **fleet verifier**, `verify_fleet(enumerated, observations)`, takes the orchestrator's live instance enumeration as a **required argument** and answers `CONSISTENT(mode)`, `MIXED` or `NOT ESTABLISHED`; complete coverage is therefore a property of the call rather than of operator discipline, and sampling traffic through the shared load balancer cannot produce that input at all. The mechanism **fails closed**: `UNKNOWN` is a distinct variant of the result type rather than a prose caveat, so it can never decay into `OFF` and a **pre-guard** instance — which has no guard mode at all — is reported `UNKNOWN`, never `MutationGuardMode::OFF`; an uncovered member, an ambiguous or empty enumeration, contradictory observations for one member, or any observation the enumeration cannot account for all yield `NOT ESTABLISHED`, and "2 of 3 instances agree" has no path to success anywhere in the API. **Silent adoption is detectable**: a declared configured intent is compared against what each process actually computed, and the fixture constructs that divergence deliberately from two genuinely real inputs — an environment variable carrying the declared intent against an explicit command-line value carrying the effective mode — so it neither depends on, reintroduces nor weakens the `init` argument-parsing defect that `THOTH-GQL-OPS-02` closed, and stays valid however the startup path is later refactored. Disclosure conforms to the **approved section 3.2 boundary** and is the smallest thing that satisfies it: **no** HTTP route, **no** listener, **no** port, **no** GraphQL field, **no** header, **no** configuration option, **no** new authorization decision, **no** third-party dependency and **no** persistent state are added, so **no public unauthenticated surface exposes the effective mode** structurally rather than by gating — proved by three negative suites that compare `GET /`, `GET /graphiql`, `GET /graphql`, `GET /schema.graphql` and `POST /graphql` **byte for byte across all three modes**, assert the absence of every mode token and of the record tag in bodies and headers, and confirm `404` for every plausible observation route. The record carries **only** the effective mode and the **minimum correlation identity** — the process's OS-reported host name, which is the orchestrator-assigned task/pod identity — justified field by field, read from the operating system rather than from an environment variable, restricted by its type to a single host-name-like token so it cannot carry a sentence, URL or configuration fragment, and **optional**: a process that cannot identify itself emits an unattributable record that verifies as `UNKNOWN` and is never defaulted. Alternatives are recorded with their dispositions — a public route or `ApiConfig` field, a public GraphQL field, an authenticated public surface, a separate administrative listener (rejected as strictly larger and unreachable per instance without a private deployment-source change), an orchestrator `exec` running a CLI that re-derives the mode in a *different* process, inferring the mode from request behaviour, and a file on disk. **Request acceptance, guard, batching and loader-store semantics are unchanged** — the `POST /graphql` handler does not appear in the diff, store availability remains derived only from the mode, the generated SDL is **byte-identical** (`sha256 1e08b46b…`, 160799 bytes at both base and head), and there is **no** migration, schema, data or public GraphQL schema change. The mechanism is **read-only, side-effect-free, concurrency-safe and stable for the process lifetime**, adds **no** work to the GraphQL request path, and needs no database or configuration. `THOTH-GQL-OPS-02` is neither reopened nor modified and its mode-control matrix is regression-pinned unchanged in **both** build profiles. Verified locally and disposably only: **no fleet was verified, no environment was transitioned, no mode was set anywhere, no deployment was performed, no deployment workflow was dispatched, no deployment credential was used, no orchestrator was queried and no secret-bearing production configuration was read** — implementing a verifier is **not** verifying a fleet. **CG-13 remains OPEN**, the `ADR-0006` runtime-operations gate remains **NOT SATISFIED**, the mode-transition runbook remains **PROVISIONAL**, `OBSERVE`, `ENFORCE` and `BE-02` runtime remain **NOT AUTHORIZED**, [PR #788](https://github.com/thoth-pub/thoth/pull/788) and [issue #765](https://github.com/thoth-pub/thoth/issues/765) are unmodified, `THOTH-GQL-OPS-04` is unimplemented with its branch absent, and the exact implementation head requires fresh independent exact-head review and separate explicit CTO merge authorization
