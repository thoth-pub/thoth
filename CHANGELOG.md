...
## [Unreleased]
### Added
  - [799](https://github.com/thoth-pub/thoth/pull/799) - `THOTH-GQL-OPS-03`: implementation candidate for the approved effective-mode **fleet-verification mechanism**. **BLOCKED — not usable as proof of any fleet's effective mode, and capability gap 2 is NOT closed.** Independent exact-head review [4906399962](https://github.com/thoth-pub/thoth/pull/799#pullrequestreview-4906399962) returned `CHANGES REQUIRED` on two load-bearing findings, both of which are **correct** and are recorded here rather than papered over. **Finding 1 — observation provenance is forgeable.** The server's pre-existing `BodyLogger` captures the raw body of every public `POST /graphql` request and the access logger interpolates it into its log line, prefixed by a newline so the attacker-controlled bytes begin their own verbatim line; that access log and the new startup effective-mode record travel the **same** `log` facade, at the **same** level, into the **same** sink. Because the parser locates its marker in arbitrary text, an **unauthenticated caller can inject record-looking content** — including a forged line prefix — that is indistinguishable from a genuine observation. This is a *provenance* defect, distinct from and additional to the (still-holding) property that no public route **returns** the mode. Trust must become structural; a longer or different marker does not fix it, and the fix is not attempted here because every candidate trust property depends on unevidenced collection behaviour. **Finding 2 — the external correlation contract is unevidenced.** The mechanism's feasibility rests on two deployment facts that no repository or local test can establish: that the serving process's output is collected with trustworthy **per-instance** provenance, and that the identity carried on an observation **correlates with the identity the orchestrator enumerates by**. A Linux process resolving a host name proves only that a host name resolves. The previous entry's claims that the OS host name *is* the orchestrator-assigned task/pod identity and that the orchestration plane *already collects* the stream per instance were **unsupported and are withdrawn**, as is the claim that **no external deployment fact is relied on** — the `THOTH-GQL-OPS-03` section 6.6 evidence boundary is **engaged**, and sanitized Route A/Route B evidence (facts E1–E5: per-instance provenance, identity correlation, generation uniqueness during rolling replacement, a channel provenance property message text cannot forge, and current-versus-terminated instance filtering) must be supplied by an **authorized human/operator or the CTO/control owner** before feasibility can be re-established. Accordingly **AC-3, AC-4, AC-11, AC-20 and AC-23 are recorded `BLOCKED`**, correcting an earlier `AC-20: PASS (not engaged)`; the overall result is `BLOCKED` rather than ready for review. Also now recorded explicitly: a record attests only that **some process computed a mode**, never that it is a **current serving member** — live orchestrator enumeration remains the sole authority on the population, and every real process in the test suite emits its record and then deliberately fails to start, which demonstrates the distinction; and emission is conditional on `info` logging, so a suppressed level yields no record, an `UNKNOWN` member and a `NOT ESTABLISHED` verification — fail-closed, never `OFF`. **What survived review and is unchanged:** the effective mode is read from the **same stored `Data<MutationGuardMode>`** the request path uses, so the reported and request-path modes are one value by construction and cannot be set independently; `OFF`, `OBSERVE` and `ENFORCE` are distinguishable; the verifier requires complete orchestrator enumeration and reduces to `CONSISTENT` / `MIXED` / `NOT ESTABLISHED`, with `UNKNOWN` a structurally distinct variant that never decays into `OFF` and a pre-guard instance reported `UNKNOWN`; incomplete or ambiguous coverage fails closed and "2 of 3 agree" has no path to success; silent adoption is detectable by comparing declared intent against the computed mode, using a fixture built from two legitimate inputs that neither depends on nor weakens the `init` defect closed by `THOTH-GQL-OPS-02`; **no public unauthenticated caller can obtain the mode** from any route, body or header, proved by byte-for-byte response comparison across all three modes; and request acceptance, guard, batching and loader-store semantics are unchanged, the generated SDL is byte-identical, and there is no migration, schema, data or public GraphQL schema change. `thoth-api-server/src/logger.rs` was inspected to reproduce the defect and **deliberately not modified** — its general request-logging behaviour is pre-existing and out of this task's scope. Verified locally and disposably only: **no fleet was verified, no environment was transitioned, no mode was set anywhere, no deployment was performed, no deployment workflow was dispatched, no deployment credential was used, no orchestrator was queried and no secret-bearing production configuration was read**. **CG-13 remains OPEN**, the `ADR-0006` runtime-operations gate remains **NOT SATISFIED**, the mode-transition runbook remains **PROVISIONAL** and now records the mechanism as not established, `OBSERVE`, `ENFORCE` and `BE-02` runtime remain **NOT AUTHORIZED**, [PR #788](https://github.com/thoth-pub/thoth/pull/788) and [issue #765](https://github.com/thoth-pub/thoth/issues/765) are unmodified, `THOTH-GQL-OPS-04` is unimplemented with its branch absent, and the exact head requires the outstanding evidence, a provenance fix, fresh independent exact-head review and separate explicit CTO merge authorization
