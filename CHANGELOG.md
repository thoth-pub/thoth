...
### Changed
  - [822](https://github.com/thoth-pub/thoth/pull/822) - `CTRL-DELIVERY-01`: reconcile the canonical repository map with re-verified live repository state (issue [819](https://github.com/thoth-pub/thoth/issues/819), parent programme [818](https://github.com/thoth-pub/thoth/issues/818)). **`thoth-pub/thoth-sphinx`** — correct the stale claims that `main` and `develop` are identical, that both contain only the placeholder `README.md`, and that `SPHINX-BOOT-01` must add a root `AGENTS.md`. Re-verified live: `main` (`0896e406`) remains the original placeholder-README-only branch, `develop` (`7d6d4a24`) is ahead by five commits with `AGENTS.md` the only changed file and the `README.md` blob unchanged, so the branches have diverged solely through the completed `CTRL-REPO-SPHINX-01` repository-control work; the repository has zero workflows and no runtime, bootstrap, CI or provider implementation, and remains bootstrap-only and non-implementation-ready. `BR-SPHINX-01` and `SPHINX-BOOT-01` remain separate, separately authorized tasks, and bootstrap must preserve and build on the existing root `AGENTS.md` rather than treat it as absent. **`thoth-pub/thoth-strapi`** — correct wording that guaranteed successful publication, separating trigger from outcome: `build_docker.yml` triggers on every PR open/update with no path filter or docs-only exemption, logs into `ghcr.io` and runs `docker/build-push-action` with `push: true` at tag `staging-pr-*`, so PR creation/update is a publication-capable external side effect that triggers an **automatic publication attempt**; successful runs publish the staging image, a run may fail before publication (both PR [#5](https://github.com/thoth-pub/thoth-strapi/pull/5) runs `31897054063` and `31898765737` concluded `failure` at `Build and push` after a successful registry login), and the actual publication result must be observed rather than assumed. The authorization requirement for PR creation/update is unchanged and not weakened, and manual `workflow_dispatch`, release publication, provider actions, deployment and activation remain distinct, separately authorized actions. Documentation and control records only: canonical repository-map corrections and the corresponding `branch-topology.md` row and re-verification note; no branch normalization, no workflow change, no manual CI action, no provider or runtime action, and no change to any other repository
