...
## [Unreleased]
### Added
  - `BE-06`: work-level Crossref upsert and Crossref write permits (issue [848](https://github.com/thoth-pub/thoth/issues/848); implementation authorization [5687443066](https://github.com/thoth-pub/thoth/issues/848#issuecomment-5687443066); specification [`BE-06-R52B.md`](docs/publisher-services/specifications/BE-06-R52B.md) as modified by Amendments 1-3). Adds migrations `20260910_v1.10.0` (the `WORK_UPSERT` kind and the `BINDING_SUPERSEDED` and `WORK_DELETED` cancellation reasons) and `20260911_v1.10.0` (source-generation capture with one commit-time flush per transaction, the execution-profile control, admission, the Crossref write permit and its DOI membership, the version floor and its audit, and the job and attempt columns and guards); the capture-driven durable residue with the bounded Crossref seed, exact generation-coverage census, exact-binding admission and the materialization drain; the work-level claim behind a per-profile execution gate; the four Crossref write reservations, finalisation with its fence, outcome reports, both voids and reconciliation with fenced-abandonment clearance; the database-local version-floor advance; publisher-set-first Work, Imprint and Publisher deletion units; 17 GraphQL mutations and 16 query fields; a scoped database-error boundary; and the export server's prepared Crossref deposit route with a supplied 17-digit timestamp. The released back-catalogue claim is bound to its own kind and cannot claim `WORK_UPSERT`. **Inert on migration and deployment: the `CROSSREF` control row starts `(capture = false, execution = false)` and the floor at `0`; no work-level job can be claimed and no Crossref traffic occurs until the separately authorized gates of R52B section 27.3. No `thoth-dissemination`, `thoth-app` or `thoth-client` change, no Metrics path, and no G-6 or G-7 execution.**
  - `BE-06-R52B-PERSIST-REPORT-01`: add the missing repository-mandated implementation report for merged PR [914](https://github.com/thoth-pub/thoth/pull/914), at [`BE-06-R52B-PERSIST-01-implementation-report.md`](docs/engineering/ai-delivery/implementation-reports/BE-06-R52B-PERSIST-01-implementation-report.md) (issue [848](https://github.com/thoth-pub/thoth/issues/848), PR [915](https://github.com/thoth-pub/thoth/pull/915), recovery authorization [5648330082](https://github.com/thoth-pub/thoth/issues/848#issuecomment-5648330082)). The report is retrospective: root `AGENTS.md` section 14 required it before review, and its absence was found by a post-ready automated review only after PR #914 had merged. It reconstructs the persistence task from durable evidence (base `395cc16a`, reviewed head `6cdb4ee4`, merge `8a711db7`, exact-head CI with runtime and image-publication jobs skipped as documentation-only) and records the control deviations without softening or backdating them: the unauthorized AI-attribution trailer on the first pushed commit and its CTO-authorized removal, and the changelog-authorization provenance gap and its later CTO retention. The approved R52B specification is not modified and remains SHA-256 `584683ca02611afb064335db83d1fb22685fc3dae87f4eeeee2e1490b320f3ce`. **Documentation/control recovery only: no runtime, schema, migration, GraphQL/API, authorization, provider, workflow, deployment or production effect; this recovery did not authorize BE-06 implementation.**
  - Persist the independently approved BE-06 R52B specification durably in the repository, at `docs/publisher-services/specifications/BE-06-R52B.md` (issue [848](https://github.com/thoth-pub/thoth/issues/848), PR [914](https://github.com/thoth-pub/thoth/pull/914)). The file is the exact byte-for-byte artifact independently approved at the specification gate by comment [5646962729](https://github.com/thoth-pub/thoth/issues/848#issuecomment-5646962729), SHA-256 `584683ca02611afb064335db83d1fb22685fc3dae87f4eeeee2e1490b320f3ce`, 699166 bytes, 5107 lines, verified unchanged after commit. No specification content is edited: in particular the LOW, non-blocking §8.7/T324 wording that calls the new BE-06 permit guard the "released permit guard" is deliberately left uncorrected, because any byte change would invalidate the exact-SHA approval; that comment records the intended reading. **Documentation/control persistence only: BE-06 implementation, migrations, GraphQL/API, Crossref/provider behaviour and runtime configuration are unaffected; no BE-06 implementation, task/slice branch, migration or source change is authorized or performed by this entry; `feature/metrics` is untouched.**
  - `CTRL-BRANCH-RELEASE-01`: preserve each managed repository's established release branch name instead of normalizing every repository to `master` (issue [897](https://github.com/thoth-pub/thoth/issues/897), parent control programme [818](https://github.com/thoth-pub/thoth/issues/818)). Adds [`ADR-0011`](docs/engineering/decisions/ADR-0011-preserve-established-release-branch-names.md), which standardizes the **role** of the release branch, written `<release-branch>`, rather than whether it is spelled `main` or `master`. The shared conceptual flows become `feature/<area>/<task> -> <development-branch> -> <release-branch>` and `feature/<programme>--<slice> -> feature/<programme> -> <development-branch> -> <release-branch>`, with each repository's `<release-branch>` resolved from its own verified live state: `main` repositories remain on `main`, `master` repositories remain on `master`, and neither name is created, renamed, moved or substituted merely because another repository uses it. `thoth`'s own `develop -> master` release flow is unchanged, and [`ADR-0009`](docs/engineering/decisions/ADR-0009-programme-integration-branch-namespace.md) is unchanged in substance. Release-branch conversion is removed from `BR-APP-01`, `BR-DIS-01`, `BR-SPHINX-01`, `BR-DASH-01`, `BR-WIDGET-01` and `BR-LIC-01`, and Vercel production for `metrics-dashboard` is no longer moved from `main` for branch spelling; the independently justified development-branch, protection, CI, provider and publication readiness work in each of those tasks is retained, CG-03 and CG-04 remain **OPEN**, and no task's risk is lowered. Reconciles `branch-topology.md`, `control-gaps.md`, the affected repository-map entries and the Metrics task tracker. **Documentation/control only: no established release, default or development-topology branch is created, deleted, renamed or moved by `ADR-0011` in any managed repository; no repository setting, default branch, protection, ruleset, CI workflow, provider or deployment configuration is changed; and there is no runtime, schema, migration, GraphQL/API, auth, contract, generated client, publication or production effect.**
...
### Fixed
  - `BE-06-R52B-PERSIST-REPORT-DURABILITY-01`: make the merged BE-06 R52B persistence implementation report ([`BE-06-R52B-PERSIST-01-implementation-report.md`](docs/engineering/ai-delivery/implementation-reports/BE-06-R52B-PERSIST-01-implementation-report.md)) durable (issue [848](https://github.com/thoth-pub/thoth/issues/848), PR [917](https://github.com/thoth-pub/thoth/pull/917), authorization [5649130376](https://github.com/thoth-pub/thoth/issues/848#issuecomment-5649130376)). This fixes Codex P2 review thread `3997644819` on PR [915](https://github.com/thoth-pub/thoth/pull/915). Section 17.9's live-state sentence that BE-06 implementation "remains unauthorized", and its branch-cleanup statements that deletion "has not been performed" or would follow once PR #915 merged, become durable historical statements: that recovery task did not authorize BE-06 implementation and did not delete either task branch, and current and future authorization and branch state are owned by GitHub. The PR #915 changelog entry's closing authorization wording is made durable in the same way. A section 18 implementation report for this task is added to the same file. All earlier history and deviations are preserved, and the approved R52B specification is not modified. **Documentation/control only: no runtime, schema, migration, GraphQL/API, authorization, provider, workflow, deployment or production effect; this correction does not itself authorize BE-06 implementation.**
