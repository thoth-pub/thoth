...
### Changed
  - [932](https://github.com/thoth-pub/thoth/pull/932) - `BR-DASH-01C-CONTROL-RECONCILIATION`: reconcile the shared engineering-control and Thoth Metrics records with the CTO-approved permanent `metrics-dashboard` topology, `feature/* -> dev -> main` (issue [931](https://github.com/thoth-pub/thoth/issues/931), parent programme [766](https://github.com/thoth-pub/thoth/issues/766)). `dev` is recorded as that repository's permanent development/integration branch and `main` as its preserved release/default branch, and no active control now requires a dashboard `dev -> develop` normalization. The cross-repository Metrics flow resolves through each repository's own verified, approved `<development-branch>` (`develop` in `thoth`, `dev` in `metrics-dashboard`), so dashboard `feature/metrics` is later created from verified `dev` and never from the stale legacy `develop` ref, which remains undeleted and separately controlled. Dashboard protection, CI/lint/test and Vercel-verification gaps remain open and `BR-DASH-01` stays HIGH; `thoth`'s own `develop -> master` flow, `ADR-0009` and `ADR-0011` are unchanged. Documentation/control only: no API, schema, runtime, authorization, provider, deployment or production change, and no default-branch, protection or ruleset change.

### Fixed
  - [934](https://github.com/thoth-pub/thoth/pull/934) - `THOTH-ONIX-ACCESS-01`: enforce the Publication accessibility slot invariant at the authoritative domain write boundary and scope e-publication accessibility detail to digital Products (issue [893](https://github.com/thoth-pub/thoth/issues/893), audit `ONIX-AUDIT-ACCESSIBILITY-01`, thoth-app#179). `PublicationProperties::validate()`, applied by the Publication create and update policies, now rejects an EPUB Accessibility or PDF/UA value in `accessibility_standard` (`AccessibilityStandardSlotError`) and a WCAG value in `accessibility_additional_standard` (`AccessibilityAdditionalStandardSlotError`) with stable domain errors, instead of leaving the refusal to the pre-existing `check_accessibility_standard_rules` database constraint's raw error; it adds no other rule, the merged database CHECK constraints remaining the authoritative accessibility contract (Architecture Amendment 3). The canonical ONIX 3.0 and 3.1 exporters emit no ProductFormFeature type `09` accessibility detail at all on print (Paperback/Hardback) Products: not the Publisher accessibility statement (List 196 `00`) or contact (`99`), the Publication's report URL (`96`), or any standard or exception code; digital Products keep their existing type-09 output and every mapping code is unchanged. No migration, schema, GraphQL shape, enum or policy change; the destination exporters and `onix21_proquest_ebrary.rs` need no correction.
