...
### Changed
  - [780](https://github.com/thoth-pub/thoth/pull/780) - Reconcile the Publisher Services control records with the proposed ADR-01 specification: record the drafted specification with independent review and CTO specification approval pending and ADR-01 implementation not authorized, record that BE-02 requires the merged ADR-01 implementation rather than the specification alone, add the programme dependency DAG and parallel thoth-app readiness track with the reserved BE-03/APP-01 GraphQL schema-pinning control, and correct the stale README claim that BE-01 implementation is blocked on THOTH-DB-CTRL-01, which contradicted the merged ADR-0003 Architecture A decision and the CG-12 RESOLVED record
