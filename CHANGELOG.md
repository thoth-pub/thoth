...
### Changed
  - [780](https://github.com/thoth-pub/thoth/pull/780) - Reconcile the Publisher Services control records with the approved ADR-01 specification: record ADR-01 as READY with its implementation branch unauthorized and the final distribution-platform inventory still provisional, record that BE-02 requires the merged ADR-01 implementation rather than the specification alone, add the coordinated backend sequence and parallel thoth-app readiness track with the reserved BE-03/APP-01 GraphQL schema-pinning control, and correct the stale README claim that BE-01 implementation is blocked on THOTH-DB-CTRL-01, which contradicted the merged ADR-0003 Architecture A decision and the CG-12 RESOLVED record
