use super::{
    AdditionalResource, AdditionalResourceField, AdditionalResourceOrderBy, NewAdditionalResource,
    NewAdditionalResourceHistory, PatchAdditionalResource,
};
...
use crate::schema::{
    additional_resource, additional_resource::dsl, additional_resource_history, work,
};
use diesel::{
...
fn validate_book_only(db: &crate::db::PgPool, work_id: &Uuid) -> ThothResult<()> {
    ...
    if work_type == WorkType::BookChapter {
        return Err(ThothError::InternalError(
            "Additional resources can only be attached to book records, not chapters".to_string(),
        ));
    }
    Ok(())
}
...
impl Crud for AdditionalResource {
    ...
    fn count(
        ...
        use crate::schema::additional_resource::dsl::*;
        let mut connection = db.get()?;
        let mut query = dsl::additional_resource
            .inner_join(work::table.inner_join(imprint::table))
            .into_boxed();
        if !publishers.is_empty() {
            query = query.filter(imprint::publisher_id.eq_any(publishers));
    ...
    fn create(db: &crate::db::PgPool, data: &Self::NewEntity) -> ThothResult<Self> {
        ...
        let mut connection = db.get()?;
        diesel::insert_into(additional_resource::table)
            .values(data)
    ...
    fn update(
        ...
        connection.transaction(|connection| {
            diesel::update(dsl::additional_resource.find(&self.pk()))
                .set(data)
    ...
    fn delete(self, db: &crate::db::PgPool) -> ThothResult<Self> {
        use diesel::{QueryDsl, RunQueryDsl};
        let mut connection = db.get()?;
        diesel::delete(dsl::additional_resource.find(&self.pk()))
...
impl Reorder for AdditionalResource {
    fn change_ordinal(
        ...
        connection.transaction(|connection| {
            ...
            diesel::sql_query("SET CONSTRAINTS idx_additional_resource_workid_ordinal DEFERRED")
                .execute(connection)?;
            for (id, ordinal) in other_objects {
                if new_ordinal > current_ordinal {
                    if ordinal > current_ordinal && ordinal <= new_ordinal {
                        ...
                        diesel::update(additional_resource::table.find(id))
                            .set(additional_resource::resource_ordinal.eq(&updated_ordinal))
                            .execute(connection)?;
                    }
            ...
            diesel::update(additional_resource::table.find(self.additional_resource_id))
                .set(additional_resource::resource_ordinal.eq(&new_ordinal))
...
impl DbInsert for NewAdditionalResourceHistory {
    type MainEntity = super::AdditionalResourceHistory;
