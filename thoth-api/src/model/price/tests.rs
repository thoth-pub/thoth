...
fn make_price(
    ...
    Price::create(pool, &new_price).expect("Failed to create price")
}
...
mod defaults {
    ...
    #[test]
    fn currencycode_default_is_gbp() {
...
mod display_and_parse {
    ...
    #[test]
    fn currencycode_display_formats_expected_strings() {
    ...
    #[test]
    fn currencycode_fromstr_parses_expected_values() {
        use std::str::FromStr;
...
mod policy {
    ...
    use crate::model::price::policy::PricePolicy;
    ...
    fn crud_policy_allows_publisher_user_for_write() {
        ...
        let user = test_user_with_role("price-user", Role::PublisherUser, &org_id);
        ...
        let price = Price::create(pool.as_ref(), &new_price).expect("Failed to create");
        let patch = PatchPrice {
        ...
        assert!(PricePolicy::can_create(&ctx, &new_price, ()).is_ok());
        assert!(PricePolicy::can_update(&ctx, &price, &patch, ()).is_ok());
        assert!(PricePolicy::can_delete(&ctx, &price).is_ok());
    }
    ...
    fn crud_policy_rejects_user_without_publisher_role() {
        ...
        let user = test_user_with_role("price-user", Role::PublisherUser, "org-other");
        let ctx = test_context_with_user(pool.clone(), user);
        ...
        assert!(PricePolicy::can_create(&ctx, &new_price, ()).is_err());
        assert!(PricePolicy::can_update(&ctx, &price, &patch, ()).is_err());
        assert!(PricePolicy::can_delete(&ctx, &price).is_err());
    }

    #[test]
    fn crud_policy_rejects_zero_unit_price() {
        let (_guard, pool) = setup_test_db();
...
mod crud {
    ...
    use crate::graphql::types::inputs::{Direction, PriceOrderBy};
    use crate::model::tests::db::{
        create_imprint, create_publication, create_publisher, create_work, setup_test_db,
        test_context,
    };
    ...
    fn crud_roundtrip_create_fetch_update_delete() {
        ...
        let price = Price::create(pool.as_ref(), &new_price).expect("Failed to create");
        let fetched = Price::from_id(pool.as_ref(), &price.price_id).expect("Failed to fetch");
        assert_eq!(price.price_id, fetched.price_id);
        ...
        let updated = price.update(&ctx, &patch).expect("Failed to update");
        assert_eq!(updated.unit_price, patch.unit_price);

        let deleted = updated.delete(pool.as_ref()).expect("Failed to delete");
        assert!(Price::from_id(pool.as_ref(), &deleted.price_id).is_err());
    }
    ...
    fn crud_all_respects_limit_and_offset() {
        ...
        let order = PriceOrderBy {
            field: PriceField::PriceId,
        ...
        assert_ne!(first[0].price_id, second[0].price_id);
    }
    ...
    fn crud_count_returns_total() {
        ...
        let count = Price::count(pool.as_ref(), None, vec![], vec![], vec![], None, None)
            .expect("Failed to count prices");
        assert_eq!(count, 2);
    }

    #[test]
    fn crud_count_filters_by_currency_code() {
        let (_guard, pool) = setup_test_db();
        ...
        .expect("Failed to count prices by currency");
        assert_eq!(count, 1);
    }

    #[test]
    fn crud_filter_param_limits_currency_codes() {
        let (_guard, pool) = setup_test_db();
        ...
        .expect("Failed to filter prices by currency");

        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0].price_id, matches.price_id);
    }
    ...
    fn crud_filter_parent_publication_id_limits_results() {
        ...
        .expect("Failed to filter prices by publication");
    ...
    fn crud_filter_publishers_limits_results() {
        ...
        .expect("Failed to filter prices by publisher");
    ...
    fn crud_ordering_by_id_respects_direction() {
        ...
        let first = make_price(
            pool.as_ref(),
            publication.publication_id,
            CurrencyCode::Usd,
            9.99,
        );
        let second = make_price(
            pool.as_ref(),
            publication.publication_id,
            CurrencyCode::Gbp,
            12.99,
        );
        let mut ids = [first.price_id, second.price_id];
        ids.sort();
        ...
        .expect("Failed to order prices (asc)");
        ...
        .expect("Failed to order prices (desc)");

        assert_eq!(asc[0].price_id, ids[0]);
        assert_eq!(desc[0].price_id, ids[1]);
    }
    ...
    fn crud_ordering_by_fields_is_supported() {
        ...
        let fields: Vec<fn() -> PriceField> = vec![
            || PriceField::PriceId,
