...
mod display_and_parse {
    ...
    #[test]
    fn languagerelation_display_formats_expected_strings() {
    ...
    #[test]
    fn languagecode_display_formats_expected_strings() {
    ...
    #[test]
    fn languagerelation_fromstr_parses_expected_values() {
        use std::str::FromStr;
        assert_eq!(
    ...
    #[test]
    fn languagecode_fromstr_parses_expected_values() {
        use std::str::FromStr;
...
mod policy {
    ...
    use crate::model::language::policy::LanguagePolicy;
    ...
    fn crud_policy_allows_publisher_user_for_write() {
        ...
        let user = test_user_with_role("language-user", Role::PublisherUser, &org_id);
        ...
        assert!(LanguagePolicy::can_update(&ctx, &language, &patch, ()).is_ok());
        assert!(LanguagePolicy::can_delete(&ctx, &language).is_ok());
    }
    ...
    fn crud_policy_rejects_user_without_publisher_role() {
        ...
        assert!(LanguagePolicy::can_update(&ctx, &language, &patch, ()).is_err());
        assert!(LanguagePolicy::can_delete(&ctx, &language).is_err());
    }
}
...
mod crud {
    ...
    use crate::graphql::types::inputs::{Direction, LanguageOrderBy};
    use crate::model::tests::db::{
    ...
    fn crud_roundtrip_create_fetch_update_delete() {
        ...
        let language = Language::create(pool.as_ref(), &new_language).expect("Failed to create");
        let fetched =
            Language::from_id(pool.as_ref(), &language.language_id).expect("Failed to fetch");
        assert_eq!(language.language_id, fetched.language_id);
        ...
        let updated = language.update(&ctx, &patch).expect("Failed to update");
        assert_eq!(updated.language_code, patch.language_code);

        let deleted = updated.delete(pool.as_ref()).expect("Failed to delete");
        assert!(Language::from_id(pool.as_ref(), &deleted.language_id).is_err());
    }
    ...
    fn crud_all_respects_limit_and_offset() {
        ...
        let order = LanguageOrderBy {
            field: LanguageField::LanguageId,
            direction: Direction::Asc,
        };

        let first = Language::all(
            pool.as_ref(),
            1,
            0,
            None,
        ...
        assert_ne!(first[0].language_id, second[0].language_id);
    }
    ...
    #[test]
    fn crud_count_filters_by_language_code() {
        let (_guard, pool) = setup_test_db();
    ...
    #[test]
    fn crud_filter_param_limits_language_codes() {
        let (_guard, pool) = setup_test_db();
        ...
        .expect("Failed to filter languages by code");

        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0].language_id, matches.language_id);
    }

    #[test]
    fn crud_filter_param_limits_language_relations() {
        let (_guard, pool) = setup_test_db();
        ...
        .expect("Failed to filter languages by relation");
    ...
    fn crud_filter_parent_work_id_limits_results() {
        ...
        .expect("Failed to filter languages by work");
    ...
    fn crud_filter_publishers_limits_results() {
        ...
        .expect("Failed to filter languages by publisher");
    ...
    fn crud_ordering_by_id_respects_direction() {
        ...
        .expect("Failed to order languages (asc)");

        let desc = Language::all(
            pool.as_ref(),
            2,
            0,
            None,
        ...
        .expect("Failed to order languages (desc)");

        assert_eq!(asc[0].language_id, ids[0]);
        assert_eq!(desc[0].language_id, ids[1]);
    }
    ...
    fn crud_ordering_by_fields_is_supported() {
        ...
        for field in fields {
            for direction in [Direction::Asc, Direction::Desc] {
                ...
                .expect("Failed to order languages");
